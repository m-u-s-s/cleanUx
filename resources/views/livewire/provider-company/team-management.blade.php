<div>
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <p class="text-xs font-bold uppercase tracking-widest text-slate-500">Gestion</p>
            <h1 class="text-2xl font-black text-slate-900">👥 Équipe</h1>
        </div>
    ...
    <div class="mb-4 flex gap-2 border-b border-slate-200">
        <button wire:click="$set('activeTab', 'members')"
            class="px-3 py-2 text-sm font-semibold transition
                {{ $activeTab === 'members' ? 'border-b-2 border-blue-500 text-slate-900' : 'text-slate-500 hover:text-slate-700' }}">
            Membres
        </button>
        <button wire:click="$set('activeTab', 'invitations')"
            class="px-3 py-2 text-sm font-semibold transition
                {{ $activeTab === 'invitations' ? 'border-b-2 border-blue-500 text-slate-900' : 'text-slate-500 hover:text-slate-700' }}">
            Invitations
        ...
        <div class="space-y-2">
                        ...
                        <p class="text-xs text-slate-500">
                            ...
                            @if ($invitation->expires_at)
                                ...
                                {{-- Une invitation expirée se lit comme une invitation en attente
                                     ...
                                     peut plus venir. --}}
                                <span class="{{ $invitation->expires_at->isPast() ? 'text-sky-600' : '' }}">
            ...
            @empty
                <div class="rounded-xl border border-dashed border-slate-200 p-8 text-center">
                    <p class="text-sm text-slate-500">Aucune invitation en attente.</p>
                </div>
            @endforelse
        </div>
    @else
    ...
    <div class="mb-4 flex flex-wrap items-center gap-3">
        <div class="relative">
            <input wire:model.live.debounce.300ms="searchQuery"
                ...
                class="w-48 rounded-xl border border-slate-200 bg-white pl-8 pr-3 py-2 text-sm text-slate-900 placeholder-slate-400 outline-none focus:border-blue-500">
            <span class="absolute left-2.5 top-2.5 text-slate-400 text-xs">🔍</span>
        </div>

        <select wire:model.live="filterRole"
            class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 outline-none focus:border-blue-500">
        ...
        <span class="ml-auto text-xs text-slate-400">{{ $members->count() }} membre(s)</span>
    </div>
        ...
        <table class="w-full min-w-[640px]">
            <thead>
                <tr class="border-b border-slate-200 bg-white/80">
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Membre</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Rôle</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Statut</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Rejoint le</th>
            ...
            <tbody class="divide-y divide-slate-200/50">
                @forelse ($members as $member)
                    ...
                    @endphp
                    <tr class="bg-white/30 hover:bg-slate-50 transition">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                <img src="{{ $member->user->profile_photo_url }}"
                                     alt="{{ $member->user->name }}"
                                     class="h-9 w-9 rounded-full object-cover border border-slate-300">
                                <div>
                                    <p class="text-sm font-semibold text-slate-900 flex items-center gap-1">
                                        {{ $member->user->name }}
                                        @if ($isSelf)
                                            <span class="text-[10px] text-slate-500">(vous)</span>
                                        @endif
                                    </p>
                        ...
                        <td class="px-4 py-3">
                            @if (! $isSelf && ! $isOwner)
                                <select wire:change="changeRole({{ $member->id }}, $event.target.value)"
                                    class="rounded-lg border border-slate-300 bg-slate-100 px-2 py-1 text-xs text-slate-700 outline-none focus:border-blue-500">
                                    @foreach ($availableRoles as $role)
                                ...
                                @if (! $isSelf && ! $isOwner)
                                    @if ($member->status === 'active')
                                        <button wire:click="suspend({{ $member->id }})"
                                            ...
                                            title="Suspendre"
                                            class="rounded-lg p-1.5 text-slate-500 hover:bg-amber-50 hover:text-sky-600">
                                            ⏸️
                                        </button>
                                    @else
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div class="w-full max-w-md rounded-2xl bg-white border border-slate-200 shadow-2xl">
            <div class="border-b border-slate-200 px-6 py-4 flex items-center justify-between">
                <h3 class="text-base font-black text-slate-900">Inviter un membre</h3>
                <button wire:click="$set('showInvite', false)" class="text-slate-500 hover:text-slate-900">✕</button>
            </div>
            ...
            <div class="flex gap-3 border-t border-slate-200 p-4">
                <button wire:click="$set('showInvite', false)"
                    class="flex-1 rounded-xl border border-slate-300 px-4 py-2 text-sm text-slate-600 hover:bg-slate-100">
                    Annuler
                </button>
            ...
            <div class="border-b border-slate-200 px-6 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">{{ $editingMember->roleLabel() }}</p>
                    </div>
                </div>
                <button wire:click="$set('showPermissions', false)" class="text-slate-500 hover:text-slate-900">✕</button>
            </div>
            <div class="p-6">
                <p class="mb-4 text-xs text-slate-500">
                    Les permissions activées ici <strong class="text-slate-700">s'ajoutent ou remplacent</strong> celles du rôle.
                </p>
                @php
                ...
                <div class="space-y-4 max-h-80 overflow-y-auto pr-1">
                    @foreach ($grouped as $domain => $perms)
                        <div>
                            <p class="mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">{{ strtoupper($domain) }}</p>
                            <div class="space-y-1">
                                @foreach ($perms as $perm)
                                    ...
                                    <div class="flex items-center justify-between rounded-lg px-3 py-2
                                        {{ $isCustomized ? 'bg-blue-50 border border-blue-500/20' : 'bg-slate-100/40' }}">
                                        <span class="text-xs text-slate-600">
                                            {{ str($perm)->after('.')->replace('_', ' ')->title() }}
                                            @if ($isCustomized)
                                                <span class="ml-1 text-[9px] text-blue-600">personnalisé</span>
                                            @endif
                                        </span>
                                        <button
            ...
            <div class="border-t border-slate-200 p-4 space-y-2">
                ...
                @if (! empty($editingMember->permissions))
                    <button wire:click="resetPermissions"
                        class="w-full rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">
