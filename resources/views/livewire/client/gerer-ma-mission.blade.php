<div class="space-y-5">
    ...
    @if(($retard['en_retard'] ?? false))
        ...
        --}}
        <div class="rounded-2xl border border-amber-200 bg-amber-50 dark:border-amber-400/30 dark:bg-amber-400/10 p-5 space-y-3" data-testid="retard-prestataire">
            <h3 class="font-semibold text-amber-900 dark:text-amber-100">{{ $retard['minutes'] }} min de retard</h3>
            ...
            @if($retard['annulation_gratuite'])
                ...
                <p class="text-xs text-amber-700 dark:text-amber-300" data-testid="retard-gratuit">
    ...
    @if($revision && $revision->attendLeClient())
        ...
        --}}
        <div class="rounded-2xl border border-indigo-200 bg-indigo-50 dark:border-indigo-400/30 dark:bg-indigo-400/10 p-5 space-y-3" wire:key="revision-{{ $revision->id }}">
            <h3 class="font-semibold text-indigo-900 dark:text-indigo-100">Nouveau devis proposé</h3>

            <div class="flex flex-wrap items-end gap-6">
                <div>
                    <p class="text-xs text-indigo-700 dark:text-indigo-300">Devis d’origine</p>
                    <p class="text-lg text-indigo-900 dark:text-indigo-100 line-through">
                        {{ number_format($revision->original_total_cents / 100, 2, ',', ' ') }} {{ $revision->currency }}
                    </p>
                </div>
                <div>
                    <p class="text-xs text-indigo-700 dark:text-indigo-300">Nouveau devis</p>
            ...
            <p class="text-sm text-indigo-900 dark:text-indigo-100">{{ $revision->reason_text }}</p>
            ...
            <div class="flex flex-wrap gap-2 pt-1">
                ...
                <button type="button" wire:click="refuserLaRevision({{ $revision->id }}, 'continue')"
                        class="rounded-lg border border-indigo-300 dark:border-indigo-400/40 px-4 py-2 text-sm font-semibold text-indigo-800 dark:text-indigo-200">
    ...
    @if($todo && $todo['engine'] !== null && $todo['engine'] !== 'vehicule')
        <div class="rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5 p-5 space-y-3">
            <h3 class="font-semibold text-slate-900 dark:text-slate-100">Ma liste de tâches</h3>

            {{-- CE QUE LA LISTE ENGAGE, DIT AVANT LE CHAMP DE SAISIE. --}}
            <p class="rounded-lg bg-amber-50 px-3 py-2 dark:bg-amber-400/10 text-xs text-amber-800 dark:text-amber-200">
            ...
            <ul class="divide-y divide-slate-100 dark:divide-white/10">
                @forelse($todo['items'] as $item)
                    <li class="flex items-center justify-between gap-3 py-2" wire:key="tache-{{ $item['id'] }}">
                        <span class="text-sm {{ $item['done'] ? 'text-slate-400 dark:text-slate-500 line-through' : 'text-slate-800 dark:text-slate-200' }}">
                            {{ $item['label'] }}
                        </span>
                        @if($item['removable'] && $todo['window']['open'])
                            <button type="button" wire:click="retirerUneTache({{ $item['id'] }})"
                                    class="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-rose-600 dark:text-rose-400"
                ...
                @empty
                    <li class="py-2 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">
            ...
            @if($todo['window']['open'])
                <form wire:submit="ajouterUneTache" class="flex flex-wrap items-end gap-2">
                    <label class="flex-1 min-w-[12rem]">
                        <span class="text-xs font-medium text-slate-600 dark:text-slate-300">Ajouter une tâche</span>
                        <input type="text" wire:model="nouvelleTache" placeholder="Nettoyer la hotte"
                               class="mt-1 w-full rounded-lg border-slate-300 dark:border-white/15 text-sm">
                    </label>
                ...
                @if(! empty($todo['suggestions']))
                    <div class="flex flex-wrap gap-2 pt-1">
                        <span class="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Souvent demandé :</span>
                        @foreach($todo['suggestions'] as $suggestion)
                            <button type="button" wire:click="$set('nouvelleTache', '{{ addslashes($suggestion) }}')"
                                    class="rounded-full bg-slate-100 px-3 py-1 dark:bg-white/10 text-xs text-slate-700 dark:text-slate-200">
                                + {{ $suggestion }}
                            </button>
                        @endforeach
                    </div>
            ...
            @else
                ...
                <p class="text-sm italic text-slate-500 dark:text-slate-400 dark:text-slate-500">{{ $todo['window']['reason'] }}</p>
            @endif
        </div>
    @endif
    ...
    <form wire:submit="enregistrerLaConsigne"
          class="rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5 p-5 space-y-3">
        <h3 class="font-semibold text-slate-900 dark:text-slate-100">Consigne d’accès de dernière minute</h3>
        ...
        <textarea wire:model="consigne" rows="2" placeholder="Le digicode est 4589."
                  class="w-full rounded-xl border-slate-300 dark:border-white/15 text-sm"></textarea>
        @error('consigne') <p class="text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
    ...
    <div class="flex flex-wrap items-center gap-3 pt-2">
        @if($ligne)
            ...
            <a href="tel:{{ $ligne['numero'] }}"
               class="inline-flex items-center rounded-lg border border-slate-300 dark:border-white/15 px-4 py-2 text-sm
                      font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50">
                Appeler
            </a>
        @endif
