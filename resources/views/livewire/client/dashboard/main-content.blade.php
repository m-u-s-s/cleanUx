<div class="grid grid-cols-1 gap-6 xl:grid-cols-3" wire:loading.remove>
    ...
    <div class="space-y-6 xl:col-span-2">
            ...
            @if($prochainRendezVous)
                <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div class="min-w-0">
                        ...
                        <h3 class="mt-4 text-xl font-bold tracking-tight text-slate-900">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div class="rounded-2xl border border-brand-200 bg-gradient-to-br from-brand-50 to-white p-6 shadow-soft-sm">
                ...
                <h3 class="mt-3 text-lg font-bold text-slate-900">{{ __('Même service que la dernière fois') }}</h3>

                @if($dernierRendezVous)
                    <dl class="mt-3 space-y-1.5 text-sm">
                        <div class="flex gap-2">
                            <dt class="text-slate-500 shrink-0">{{ __('Service') }} :</dt>
                            <dd class="font-medium text-slate-800 truncate">{{ $dernierRendezVous->service_display_name }}</dd>
                        </div>
                        <div class="flex gap-2">
                            <dt class="text-slate-500 shrink-0">{{ __('Adresse') }} :</dt>
                            <dd class="font-medium text-slate-800 truncate">{{ $dernierRendezVous->adresse ?? '—' }}, {{ $dernierRendezVous->ville ?? '—' }}</dd>
                        </div>
                        <div class="flex gap-2">
                            <dt class="text-slate-500 shrink-0">{{ __('Fréquence') }} :</dt>
                            <dd class="font-medium text-slate-800">{{ ucfirst(str_replace('_', ' ', $dernierRendezVous->frequence ?? '—')) }}</dd>
                        </div>
                    </dl>
        ...
        <x-app-card padding="p-6" wire:loading.class="opacity-60">
            ...
            <div class="mt-5 space-y-3">
                @forelse($avenir as $rdv)
                    <div class="rounded-xl border border-slate-200/70 bg-white p-4 transition hover:border-slate-300 hover:shadow-soft-sm">
                        ...
                        @if($rdv->mission?->report_path)
                            <div class="mt-3 border-t border-slate-100 pt-3">
                                <a href="{{ asset('storage/'.$rdv->mission->report_path) }}" target="_blank"
    ...
    <div class="space-y-6">
        ...
        <x-app-card padding="p-6" :title="__('Profil & couverture')" :subtitle="__('Résumé de votre compte.')">
            <div class="space-y-2.5">
                <div class="flex items-center justify-between rounded-lg border border-slate-200/70 bg-slate-50/50 p-3">
                    <span class="text-xs text-slate-500">{{ __('Type de compte') }}</span>
                    <span class="text-sm font-semibold text-slate-900">{{ $accountContext['type_label'] ?? __('Standard') }}</span>
                </div>

                <div class="flex items-center justify-between rounded-lg border border-slate-200/70 bg-slate-50/50 p-3">
                    <span class="text-xs text-slate-500">{{ __('Zone principale') }}</span>
                    <div class="text-right">
                        <p class="text-sm font-semibold text-slate-900">{{ $accountContext['primary_zone'] ?? __('Non définie') }}</p>
                        @if(($accountContext['zone_count'] ?? 0) > 1)
                            <p class="text-xs text-slate-500">{{ $accountContext['zone_count'] }} {{ __('zones') }}</p>
                        @endif
                    </div>
                </div>
                ...
                @if($accountContext['organization_name'] ?? false)
                    <div class="flex items-center justify-between rounded-lg border border-slate-200/70 bg-slate-50/50 p-3">
                        <span class="text-xs text-slate-500">{{ __('Organisation') }}</span>
                        <span class="text-sm font-semibold text-slate-900">{{ $accountContext['organization_name'] }}</span>
                    </div>
                @endif
            </div>
        </x-app-card>
        ...
        <x-app-card padding="p-6" :title="__('Documents & finance')" :subtitle="__('Devis, factures et reste à payer.')">
            <div class="grid grid-cols-2 gap-2.5">
                <div class="rounded-lg border border-slate-200/70 bg-slate-50/50 p-3">
                    <p class="text-xs text-slate-500">{{ __('Devis') }}</p>
                    <p class="mt-0.5 text-xl font-bold text-slate-900">{{ $financeSnapshot['quotes_count'] ?? 0 }}</p>
                </div>

                <div class="rounded-lg border border-slate-200/70 bg-slate-50/50 p-3">
                    <p class="text-xs text-slate-500">{{ __('Factures') }}</p>
                    <p class="mt-0.5 text-xl font-bold text-slate-900">{{ $financeSnapshot['invoices_count'] ?? 0 }}</p>
                </div>

                <div class="rounded-lg border border-rose-200/70 bg-rose-50/50 p-3">
                    <p class="text-xs text-rose-600">{{ __('En retard') }}</p>
                    <p class="mt-0.5 text-xl font-bold text-rose-700">{{ $financeSnapshot['overdue_count'] ?? 0 }}</p>
                </div>

                <div class="rounded-lg border border-emerald-200/70 bg-emerald-50/50 p-3">
                    <p class="text-xs text-emerald-600">{{ __('À payer') }}</p>
        ...
        <x-app-card padding="p-6" :title="__('Services disponibles')" :subtitle="__('Selon votre zone.')">
            <div class="space-y-2">
                @forelse($availableServices as $service)
                    <div class="flex items-center justify-between gap-3 rounded-lg border border-slate-200/70 bg-white p-3">
                        <div class="min-w-0">
                            <p class="text-sm font-semibold text-slate-900 truncate">{{ $service['name'] }}</p>
                            <p class="text-xs text-slate-500 truncate">{{ $service['zone_name'] ?: __('Zone couverte') }}</p>
                        </div>
                    ...
                    @forelse($favoriteEmployes as $employe)
                        <div class="flex items-center justify-between gap-3 rounded-lg border border-slate-200/70 bg-white p-3">
                            <div class="flex items-center gap-2.5 min-w-0">
                                <div class="grid h-9 w-9 place-items-center rounded-full bg-amber-100 text-amber-700 font-bold text-xs shrink-0">
                                    {{ strtoupper(substr($employe->name, 0, 1)) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-slate-800 truncate">{{ $employe->name }}</p>
