    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <x-app-card class="xl:col-span-1" padding="p-5 md:p-6" title="Créer / modifier" subtitle="Définissez les paramètres clés du service sélectionné.">
            ...
            <div class="mt-5 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2 xl:grid-cols-1">
                <label class="brio-choice-card !items-center"><input wire:model.defer="is_active" type="checkbox"> <span>Actif</span></label>
                <label class="brio-choice-card !items-center"><input wire:model.defer="requires_quote" type="checkbox"> <span>Sur devis</span></label>
                <label class="brio-choice-card !items-center"><input wire:model.defer="requires_manual_validation" type="checkbox"> <span>Validation manuelle</span></label>
                <label class="brio-choice-card !items-center"><input wire:model.defer="is_entreprise" type="checkbox"> <span>Entreprise</span></label>
            </div>

            <div class="mt-5 flex flex-wrap gap-3">
                <button wire:click="saveService" class="brio-btn-primary">Enregistrer le service</button>
                <button wire:click="resetServiceForm" class="brio-btn-secondary">Réinitialiser</button>
            </div>
        </x-app-card>

        <div class="xl:col-span-2 space-y-6">
            <div class="brio-filter-panel space-y-4">
                <div class="brio-toolbar gap-3">
                    <div>
                        <h3 class="brio-section-title">Filtres catalogue</h3>
                        <p class="brio-section-subtitle">Recherchez rapidement les services et segmentez par type, statut ou marché.</p>
                    </div>
                </div>
            ...
            <div wire:loading.delay.short wire:target="search,status,market,serviceType,tradeFilter,groupByTrade,selectService,toggleActive" class="brio-loading-panel space-y-3">
                                            ...
                                            <tr class="{{ $selectedService?->id === $service->id ? 'bg-sky-50/70' : '' }}">
                                                ...
                                                <td>
                                                    @if($service->is_active)
                                                        <span class="brio-chip !border-emerald-200 !bg-emerald-50 !text-emerald-700">Actif</span>
                                                    @else
                                                        <span class="brio-chip !border-red-200 !bg-red-50 !text-red-700">Inactif</span>
                                                    @endif
                                                </td>
                                                <td class="text-right">
                                                    <div class="flex justify-end gap-2">
                                                        <button wire:click="selectService({{ $service->id }})" class="brio-btn-secondary">Gérer</button>
                                                        <button wire:click="toggleActive({{ $service->id }})" class="brio-btn-secondary">Basculer</button>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
                        ...
                        <tbody>
                            @forelse($services as $service)
                                <tr class="{{ $selectedService?->id === $service->id ? 'bg-sky-50/70' : '' }}">
                                    ...
                                    <td>
                                        @if($service->trade)
                                            <span class="brio-chip !border-blue-200 !bg-blue-50 !text-blue-700">{{ $service->trade->name }}</span>
                                        @else
                                            <span class="brio-chip !border-amber-200 !bg-amber-50 !text-amber-700">Non rattaché</span>
                                        @endif
                                    </td>
                                    <td>{{ $service->service_type }}</td>
                                    <td><x-money :amount="(float) ((float) $service->base_price)" /></td>
                                    ...
                                    <td>
                                        @if($service->is_entreprise)
                                            <span class="brio-chip !border-purple-200 !bg-purple-50 !text-purple-700">Entreprise</span>
                                        @else
                                            <span class="brio-chip">Particulier</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($service->is_active)
                ...
                <x-app-card padding="p-5 md:p-6" title="Règles par zone · {{ $selectedService->name }}" subtitle="Ajustez l'activation, la tarification et la validation zone par zone.">
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                        ...
                        <div class="lg:col-span-2 flex items-end">
                            @if($selectedZoneId)
                                <button wire:click="editZoneRule({{ (int) $selectedZoneId }})" class="brio-btn-secondary">Charger la règle</button>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mt-5 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                        <label class="brio-choice-card !items-center"><input wire:model.defer="rule_enabled" type="checkbox"> <span>Service actif sur la zone</span></label>
                        <label class="brio-choice-card !items-center"><input wire:model.defer="rule_requires_manual_validation" type="checkbox"> <span>Validation manuelle sur la zone</span></label>
                    </div>

                    <div class="mt-5 flex gap-3">
                        <button wire:click="saveZoneRule" class="brio-btn-primary">Enregistrer la règle</button>
                        <button wire:click="resetRuleForm" class="brio-btn-secondary">Réinitialiser</button>
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($rule->is_enabled)
                                                    <span class="brio-chip !border-emerald-200 !bg-emerald-50 !text-emerald-700">Actif</span>
                                                @else
                                                    <span class="brio-chip !border-red-200 !bg-red-50 !text-red-700">Bloqué</span>
                                                @endif
                                            ...
                                            <td class="text-right">
                                                <button wire:click="editZoneRule({{ $rule->service_zone_id }})" class="brio-btn-secondary">Éditer</button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                ...
                <x-app-card padding="p-5 md:p-6" title="Options du service · {{ $selectedService->name }}" subtitle="Variantes paramétrables (m², options, fréquences…) qui modifient le prix ou la durée.">
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-3">
                        ...
                        <div class="flex gap-2 justify-end">
                            <button wire:click="resetNewOption" class="brio-btn-secondary">Réinitialiser</button>
                            <button wire:click="addOption" class="brio-btn-primary">Ajouter l'option</button>
                        </div>
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($opt['price_modifier'] !== 'none')
                                                    <span class="brio-chip !border-blue-200 !bg-blue-50 !text-blue-700">
                                            ...
                                            <td>
                                                @if($opt['is_required'])
                                                    <span class="brio-chip !border-amber-200 !bg-amber-50 !text-amber-700">Oui</span>
                                                @else
                                            ...
                                            <td>
                                                @if($opt['is_active'])
                                                    <span class="brio-chip !border-emerald-200 !bg-emerald-50 !text-emerald-700">Active</span>
                                                @else
                                                    <span class="brio-chip !border-red-200 !bg-red-50 !text-red-700">Inactive</span>
                                                @endif
                                            </td>
                                            <td class="text-right">
                                                <div class="flex justify-end gap-2">
                                                    <button wire:click="editOption({{ $optionId }})" class="brio-btn-secondary">Éditer</button>
                                                    <button wire:click="toggleOptionActive({{ $optionId }})" class="brio-btn-secondary">Basculer</button>
                                                    <button wire:click="deleteOption({{ $optionId }})" class="brio-btn-secondary text-rose-700"
                                                ...
                                                <td colspan="7">
                                                    <div class="grid grid-cols-1 gap-3 md:grid-cols-3 p-3">
                                                        ...
                                                        <div class="md:col-span-3 flex justify-end gap-2">
                                                            <button wire:click="cancelEditOption" class="brio-btn-secondary">Annuler</button>
                                                            <button wire:click="saveOption({{ $optionId }})" class="brio-btn-primary">Enregistrer</button>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
