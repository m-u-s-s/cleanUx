<div class="mx-auto max-w-4xl px-4 py-6">
    ...
    <header class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Demande multi-locaux</h1>
    ...
    <div class="mb-6 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 dark:border-emerald-700 dark:bg-emerald-900/30">
        ...
        <a href="{{ route('client-company.bookings.index') }}"
            class="mt-1 inline-block text-sm font-semibold text-emerald-700 underline dark:text-emerald-300">
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        ...
        <fieldset class="mb-5">
            <legend class="mb-2 text-sm font-bold text-slate-900 dark:text-white">Locaux concernés</legend>

            @forelse ($sites as $site)
            <label class="mb-1 flex cursor-pointer items-center gap-3 rounded-lg border border-slate-200 px-3 py-2 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-700/40">
                <input type="checkbox" wire:model="siteIds" value="{{ $site->id }}" class="rounded">
                <span class="min-w-0 flex-1">
                    <span class="block truncate text-sm font-medium text-slate-900 dark:text-white">{{ $site->name }}</span>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <label class="block">
                <span class="mb-1 block text-sm font-semibold text-slate-900 dark:text-white">Prestation</span>
            ...
            <label class="block">
                <span class="mb-1 block text-sm font-semibold text-slate-900 dark:text-white">Durée par local (min)</span>
        ...
        <label class="mt-4 block">
            <span class="mb-1 block text-sm font-semibold text-slate-900 dark:text-white">Précisions (facultatif)</span>
            <textarea wire:model="commentaire" rows="3"
                class="w-full rounded-lg border-slate-300 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-white"></textarea>
        </label>
