...
/** Deux chemins de création (CreateBookingFromApiAction et ProcessRecurringBookings) ne renseignaient aucune destination, contrairement à la synchronisation depuis une réservation, qui géocode l'adresse. */
class GeocodeMissionDestinationTest extends TestCase
{
    ...
    /** Comportement observé sur le vrai Nominatim : « 10 Rue des Arts, 1000, Bruxelles, BE » ne rend AUCUN résultat, alors que « 1000, Bruxelles, BE » rend le centroïde. */
    ...
    /** Câblage : sans cette mise en file, une mission créée par le chemin API n'obtient jamais de destination, et l'offre arrive sur la carte du prestataire sans marqueur. */
    public function test_creating_an_asap_booking_queues_the_geocoding_of_its_mission(): void
    {
        Queue::fake();
        $user = User::factory()->create();

        // LE CATALOGUE DOIT ÊTRE OUVERT POUR L'IMMÉDIAT, sinon la création refuse avant même d'arriver au géocodage.
    ...
    /** withoutEvents : un booking `confirme` réveille RendezVousObserver, qui crée la mission de la réservation et géocode son adresse. */
