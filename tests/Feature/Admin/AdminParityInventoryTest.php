...
/** La parité mobile se MESURE, elle ne se déclare pas. POURQUOI CE FICHIER EXISTE. */
class AdminParityInventoryTest extends TestCase
{
    // UNE BASE EST NÉCESSAIRE, et ce n'est pas un détail de test : certains descripteurs construisent leurs listes déroulantes en INTERROGEANT la base — les pays d'un filtre, les secteurs d'un formulaire.
    use RefreshDatabase;

    /** Les modules dont le web n'offre aucun geste, et pour lesquels le mobile n'a donc rien à porter : une liste reste une liste. */
    ...
    {
        // Le mode d'échec le plus discret d'un test de ce genre : l'heuristique cesse de repérer quoi que ce soit — un remaniement des composants, un format d'écriture différent — et le test passe au vert pour la pire des raisons.
        $total = 0;
