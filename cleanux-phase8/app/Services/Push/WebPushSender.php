 ...
 *
 * Wrapper autour de minishlink/web-push (composer require minishlink/web-push).
 *
 * Usage :
 ...
 *   - Désactivation des subscriptions invalides (HTTP 404/410)
...
{
    /**
     * @param array{title:string, body:string, url?:string, icon?:string, tag?:string, image?:string, requireInteraction?:bool, actions?:array, data?:array} $payload
    ...
    /**
     * Envoie à plusieurs users (broadcast).
     */
    ...
    /**
     * Coeur de l'envoi : itère sur les subscriptions et envoie via web-push.
     */
    protected function sendToSubscriptions($subscriptions, array $payload): array
    {
        ...
        // Map subscription_id → modèle pour traiter les rapports après
        $subById = [];

        foreach ($subscriptions as $sub) {
            $webPushSub = \Minishlink\WebPush\Subscription::create($sub->toWebPushArray());
            $report = $webPush->queueNotification($webPushSub, $payloadJson);
            // queueNotification ne retourne pas immédiatement, on le récupère via flush()
        ...
        foreach ($webPush->flush() as $report) {
            ...
            if ($report->isSuccess()) {
                ...
                // 404/410 = endpoint mort (user a désinstallé/désactivé) → désactiver
    ...
    /**
     * Garantit que le payload a les champs minimum + valeurs par défaut.
     */
