{{--
  ...
  Comportement :
    - Sur Android/Chrome/Edge : utilise beforeinstallprompt (bouton Installer)
    - Sur iOS Safari : affiche les instructions manuelles (Partager → Sur l'écran d'accueil)
    - Si déjà installé (mode standalone) : ne montre rien
    - Persistance dismiss en localStorage ("cleanux_pwa_dismissed")
--}}
...
<div x-data="pwaInstallPrompt()"
    ...
    <div x-show="mode === 'native'" class="rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
        <div class="flex items-start gap-3">
            <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-blue-100 text-2xl">
                📱
            </div>
...
<script>
    function pwaInstallPrompt() {
        return {
            showPrompt: false,
            mode: null,        // 'native' ou 'ios'
            dismissedKey: 'cleanux_pwa_dismissed_v1',

            init() {
                // Si déjà installé : skip
                if (window.cleanuxPwa?.isStandalone()) return;

                // Si dismissed récemment : skip 30 jours
                ...
                // iOS Safari : pas d'install API native, montrer les instructions après 5s
                ...
                // Chrome/Edge/Android : attendre l'event 'pwa:install-available'
                window.addEventListener('pwa:install-available', () => {
                    this.mode = 'native';
                    this.showPrompt = true;
                });

                // Si le bouton est déjà disponible au load (ex: nav SPA)
